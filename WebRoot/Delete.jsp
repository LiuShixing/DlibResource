...
<%
  ...
  //  if(!wiki.getAuthorizationManager().hasAccess( wikiContext, response )) return;
    ...
    //*************************************************************************************
			...
			{ //System.out.println("deleteFileName 1");
				
				if(app !=null)
				{//System.out.println("deleteFileName:pagereq="+pagereq);
					Map<String,ResourceInfo> mp=(Map<String,ResourceInfo>) app.getAttribute(MyUtil.RESOURCE_INFO_MP);
					
					ResourceInfo info = mp.get(pagereq.toLowerCase());
					if(info!=null)
					{
						String deleteFileName = info.getName();
					//	System.out.println("deleteFileName"+deleteFileName);
						File deleteFile = new File(MyUtil.RESOURCE_DIR+"/"+info.getID()+"/"+ deleteFileName);
						
						if(deleteFile.isFile() && deleteFile.exists())
						{
						   if(deleteFile.delete())
						   {
						  		File deleteDir = new File(MyUtil.RESOURCE_DIR+"/"+info.getID());
						  		deleteDir.delete();
						        mp.remove(pagereq);
								//保存mp
					    		app.setAttribute(MyUtil.RESOURCE_INFO_MP, mp);
			...
			sql="delete from article_labels where artid=?";
     ...
     //***********************************************************************************
        ...
        if(tab!=null && tab.equals("attach"))
        {
            response.sendRedirect("Edit.jsp?page="+redirTo);
            return;
        }
        response.sendRedirect("Home.jsp");
      //  response.sendRedirect( TextUtil.replaceString( wiki.getURL( WikiContext.VIEW, redirTo, "", false ),"&amp;","&" ));
        return;
    }
    else if( delete != null )
    {
        ...
        TextUtil.replaceString(wiki.getURL(WikiContext.VIEW, redirTo, "", false),"&amp;","&")
        );
