...
<div class="resourceList">
 ...
 <%
		...
		while(it.hasNext())
		{
		   ...
		   if(!wikiEngine.pageExists( info.getID() ))
           {
			...
			if(fullName.equals(info.getAuthor()))
			{
				if((resType==null || resType.equals("all") ||  resType.equals("null")) ||m.get(info.getType()).equals(resType) )
				{ 
					needRes.add(info);
				}
			}
 ...
 <%
  ...
  %>
    ...
    <td class="resourceNameClass"><a class="resourceListType" href="<%=goPage %>&resType=<%=m.get(info.getType()) %>#section-personalUpload">[<%=m_name.get(info.getType()) %>]</a>
...
if(total >itemsPerPage)
{
     ...
     {
 %>
  <a href="<%=goPage %>&resType=<%=resType %>&currPage=<%=currPage-1 %>#section-personalUpload">
 ...
 <%
      for(int p:pageArr)
      {
           ...
           {
  %>
             <a href="<%=goPage %>&resType=<%=resType %>&currPage=<%=p %>#section-personalUpload" >
...
<a href="<%=goPage %>&resType=<%=resType %>&currPage= " id="jumpToLink">
 <input type="button" value="跳转" class="mybutton myblue" onclick="
	    ...
	    if(!(Math.floor(page) == page) || page > <%=totalPage %> || page < 1 ){
	        a.href='#';
	        document.getElementById('jumpToText').value=<%=currPage %>;
	        alert('没有该页');
	       
	    }else{
	          a.href+=page;
	          a.href+='#section-personalUpload';
	    }
